...
export class CosmicCardioScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.x1Pos = xOffsett + 500;
        this.x2Pos = xOffsett + 505;
    ...
    generateFakeStocksData() {
        ...
        this.avgPrice = priceData.map(p => p.y).reduce((a, b) => a + b, 0) / priceData.lenght
        this.startingPrice = this.curPrice  * 0.5;
    }
    ...
    update(time, delta) {
        ...
        // const x2Pos = xOffsett + 505;
        const nextPrice = {
            x: this.priceData[this.priceData.length-1].x + chartTimeInterval, y: this.curPrice
        }
        // this.graphics.lineTo(xOffsett + nextPrice.x, nextPrice.y - height * 0.5);
        ...
        // maybe
        if (Date.now() - this.frameTime > 10000) {
            this.x1Pos += 5;
            this.x2Pos +=  5;
            this.frameTime = Date.now();
        }
