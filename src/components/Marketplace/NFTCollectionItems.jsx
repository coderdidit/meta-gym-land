...
function NFTCollectionItems({ nftAddress, colName }) {
  ...
  const {
    ...
    isLoading,
  } = useNFTTokenIds(nftAddress, maxItems, marketPlaceChainId);
  ...
  const queryMarketItems = useMoralisQuery(createdMarketItemsTable, (query) => {
    // get not sold items
    return query
      .equalTo("sold", false)
  ...
  const getMarketItems = (nft) => {
    const result = fetchMarketItems.filter(
      (e) =>
        e.nftContract === nft?.token_address && e.tokenId === nft?.token_id,
    );
  ...
  if (isLoading) {
    ...
    return (
      <>
        ...
        <div style={NFTsDiv}>
          {NFTTokenIds &&
            shuffle(NFTTokenIds.result).map((nft, index) => {
              //Verify Metadata
                  ...
                  actions={[
                    <Tooltip title="View On Blockexplorer">
                      <FileSearchOutlined
                        onClick={() =>
                          window.open(
                            `${getExplorer(marketPlaceChainId)}address/${
                              nft.token_address
                            }`,
                            "_blank",
                          )
                      ...
                      <>
                        ...
                        >
                          ...
                          <h1>
                            ...
                            &nbsp;<b style={{ color: "#5740C1" }}>/</b>&nbsp;
                            <b style={{ color: "#FF74A6" }}>
                              {getAmountForSale(nft)}
                            </b>
                          </h1>
                          <div>
                            {hasMarketItems(nft) ? (
                              <h4
                                style={{
                                  background:
                                    "linear-gradient(90deg, #83B4FF 0%, #FFA2C4 96.53%)",
                                  borderRadius: "1rem",
                                  padding: "2px",
                                }}
                              >
                                <b style={{ color: "#fff" }}>
                                  Best Price&nbsp;
                                  {getMarketWithLowestPrice(nft).price /
                                    ("1e" + 18)}
                                </b>
                                &nbsp;
              ...
              <Spin spinning={loading}>
                ...
                >
                  <Badge.Ribbon
                    color="green"
                    text={`${
                      getMarketWithLowestPrice(nftToBuy).price / ("1e" + 18)
                    } ${nativeName}`}
                    style={{
                      marginRight: "-100px",
                    }}
